    ...
    if grep -q "ODROID-N2" /sys/firmware/devicetree/base/model 2>/dev/null; then
        if [[ "$SYSTEM" == "kodi" ]]; then
            ...
            if [ $rescheck -eq 0 ]; then
                dialog --no-ok --no-cancel --pause 'IMPORTANT NOTE\n\nKodi Leia 18.3 requires the HDMI resolution set to 1080p.\n\nTo enable, launch Odroid Config from Options in EmulationStation.\n\nPlease note that 720p will perform better in majority of 3D games than 1080p. There is an option to revert back to 720p.' 22 76 15
                clear
                user_script "runcommand-onend.sh"
                exit 1
            fi
