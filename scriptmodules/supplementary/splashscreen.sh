...
function default_splashscreen() {
    echo "$md_inst/retroarena-default.png" >/etc/splashscreen.list
}
...
function randomize_splashscreen() {
    options=(
        1 "Randomize RetroArena splashscreens"
...
function gui_splashscreen() {
    ...
    while true; do
        ...
        options+=(
            ...
            8 "Update RetroArena splashscreens"
        )
        local choice=$("${cmd[@]}" "${options[@]}" 2>&1 >/dev/tty)
        if [[ -n "$choice" ]]; then
            case "$choice" in
                ...
                4)
                    ...
                    printMsgs "dialog" "Splashscreen set to RetroArena default."
                    ;;
                5)
