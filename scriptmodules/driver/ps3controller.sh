...
rp_module_licence="GPL2 https://raw.githubusercontent.com/RetroArena/sixad/master/COPYING"
...
function sources_ps3controller() {
    ...
    gitPullOrClone "$md_build/sixad" https://github.com/Retro-Arena/sixad.git $branch
}
...
function gui_ps3controller() {
    ...
    printMsgs "dialog" "WARNING: The ps3controller driver partially disables the standard Bluetooth stack so that Dual Shock controllers can pair correctly. Although the Bluetooth stack is temporarily re-enabled inside RetroArena's Bluetooth menu to allow compatibility with standard Bluetooth peripherals, any other software that relies on the full Bluetooth stack will not work correctly while the ps3controller driver is active."
