...
function configure_uae4arm() {
    ...
    for system in amiga amigacd32; do
        ...
        mkUserDir "$md_conf_root/$system/$md_id"
        
        # move config / save folders to $md_conf_root/$system/$md_id
        local dir
        for dir in conf savestates screenshots; do
            moveConfigDir "$md_inst/$dir" "$md_conf_root/$system/$md_id/$dir"
        done
        ...
        local conf="$(mktemp)"
        iniConfig "=" "" "$conf"
        iniSet "config_description" "RetroArena A500, 68000, OCS, 512KB Chip + 512KB Slow Fast"
        iniSet "chipmem_size" "1"
        iniSet "bogomem_size" "2"
        iniSet "chipset" "ocs"
        iniSet "cachesize" "0"
        iniSet "kickstart_rom_file" "\$(FILE_PATH)/kick13.rom"
        copyDefaultConfig "$conf" "$md_conf_root/$system/$md_id/conf/rp-a500.uae"
        rm "$conf"
        
        conf="$(mktemp)"
        iniConfig "=" "" "$conf"
        iniSet "config_description" "RetroArena A1200, 68EC020, AGA, 2MB Chip"
        ...
        copyDefaultConfig "$conf" "$md_conf_root/$system/$md_id/conf/rp-a1200.uae"
        rm "$conf"
    ...
    _EOF_
        ...
        addEmulator 1 "$md_id" "$system" "$md_inst/$md_id.sh auto %ROM%"
        addEmulator 1 "$md_id-a500" "$system" "$md_inst/$md_id.sh rp-a500.uae %ROM%"
        addEmulator 1 "$md_id-a1200" "$system" "$md_inst/$md_id.sh rp-a1200.uae %ROM%"
