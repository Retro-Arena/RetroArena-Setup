...
function gui_lr-flycast() {
    while true; do
        local options=()
            [[ -e "$home/.config/au_lr-flycast" ]] && options+=(A "Disable lr-flycast AutoUpdate (Daily)") || options+=(A "Enable lr-flycast AutoUpdate")
        ...
        case "$choice" in
            A)
                if [[ -e "$home/.config/au_service" ]]; then
                    if [[ -e "$home/.config/au_lr-flycast" ]]; then
                        rm -rf "$home/.config/au_lr-flycast"
