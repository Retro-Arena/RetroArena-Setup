...
function gui_lr-genesis-plus-gx() {
    while true; do
        local options=()
            [[ -e "$home/.config/au_lr-genesis-plus-gx" ]] && options+=(A "Disable lr-genesis-plus-gx AutoUpdate (Daily)") || options+=(A "Enable lr-genesis-plus-gx AutoUpdate")
        ...
        case "$choice" in
            A)
                if [[ -e "$home/.config/au_service" ]]; then
                    if [[ -e "$home/.config/au_lr-genesis-plus-gx" ]]; then
                        rm -rf "$home/.config/au_lr-genesis-plus-gx"
                        printMsgs "dialog" "Disabled lr-genesis-plus-gx AutoUpdate"
                    else
                        touch "$home/.config/au_lr-genesis-plus-gx"
                        printMsgs "dialog" "Enabled lr-genesis-plus-gx AutoUpdate\n\nThe update will occur daily at 05:00 UTC."
                    fi
                else
